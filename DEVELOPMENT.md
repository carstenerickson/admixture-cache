...
| LD-prune `window_size` / `step_size` / `r2_threshold` | `50 / 5 / 0.5` | Standard ADMIXTURE manual recommendation: a 50-variant window, 5-variant step, r²<0.5 (a plink2 `--indep-pairwise` window is a variant count, not kb). 30-50% variant retention on typical panels gives 3-5× total speedup. (`window_size` was named `window_kb` before; the old keyword still works as a deprecated alias.) |
